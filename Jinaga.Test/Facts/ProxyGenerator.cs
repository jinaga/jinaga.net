...
public class ProxyGenerator
{
    public static T CreateProxy<T>(T instance)
    {
        ...
        // TODO: Generate the fact reference
        FactReference factReference = new FactReference("TODO", "hash");
    ...
    private static TypeBuilder GetTypeBuilder(Type type)
    {
        ...
        // Define a backing field for the reference
        var fieldBuilder = tb.DefineField(
            "reference",
            typeof(FactReference),
            FieldAttributes.Private
        );
        // Define a property for the FactReference
        var propertyBuilder = tb.DefineProperty(
            nameof(IFactProxy.Reference),
        ...
        // Implement the getter for the FactReference
        ...
        var parameters = constructor.GetParameters();
        // Define a constructor for the proxy that takes all of the parameters for T plus a FactReference
        var constructorBuilder = tb.DefineConstructor(
            MethodAttributes.Public,
            CallingConventions.Standard,
            parameters.Select(p => p.ParameterType).Append(typeof(FactReference)).ToArray()
        );
