...
namespace Jinaga
{
    ...
    public class PurgeConditions
    {
        ...
        public static string Describe(Func<PurgeConditions, PurgeConditions> builder)
        {
            var purgeConditions = builder(PurgeConditions.Empty);
            return purgeConditions.SaveToDescription();
        }

        private string SaveToDescription()
        {
            var description = new StringBuilder();
            description.AppendLine("purge {");
            foreach (var specification in specifications)
            {
                description.Append(specification.ToDescriptiveString(1));
            }
